...
- [~] Nova fonte `categoria='lista_urls'`: planilha dedicada (colunas URL, Nome) para páginas institucionais avulsas (fora do calendário de cursos) — pedido do Raphael em 2026-09-14. `parseListaUrls` e `ingestListaUrls` implementados e deployados, reaproveitando `syncSourcesFromUrlList` (generalizado a partir da lógica de páginas de curso). **Falta**: Raphael criar a planilha "Copiloto | Páginas Institucionais" e passar o ID para cadastro em `sources` — sem isso ainda não há nenhuma fonte dessa categoria ativa.
