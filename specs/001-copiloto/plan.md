...
- **Dívida de arquitetura do RF07 — fechada na Etapa 14 (2026-09-16):** a planilha oficial de preços (PGL/GRL) chegou e virou uma tabela estruturada dedicada, `precos_cursos` (não `facts` genérico — o layout real, com semana de vigência e as duas colunas de valor final, pedia uma tabela própria). Nova função `buscar_preco_curso(produto, data)` faz o match determinístico e pega sempre a semana vigente (a planilha já cadastra semanas futuras com antecedência). `suggest`/`ask` (via `_shared/precos.ts`) detectam o curso pelos próprios chunks de `calendario_cursos` já recuperados, chamam essa função e `buscar_convenio_empresa` diretamente, e injetam um bloco "DADOS OFICIAIS DE PREÇO" no prompt — o LLM nunca mais soma/calcula desconto sozinho, só repete os números prontos (incluindo as 5 formas de pagamento de PGL e a mensalidade de GRL, com multiplicadores fixos numa tabela `formas_pagamento_cursos` editável sem deploy). A fonte de teste com valores hipotéticos (`tasks.md` etapa 8) foi removida.
